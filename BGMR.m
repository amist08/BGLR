function [W,lossFun]=BGMR(X,k,m,h,beta,eta,maxIter)
   %% input:
       %    data matrix:X,dxn
       %    feature number:k
...
%              U2=M2*pinv(M2'*M2)*M2';
         ...
         Losspicture1(loss,20);
       end
%        %% update Z
%        [Z]=get_Bipartite(W'*X,cls_num);%更新二部图、锚点
%        %% 计算损失函数
%        disp('calculate loss')
...
%        if(i>10)
%            if(abs((lossFun(i)-lossFun(i-1)))/lossFun(i)<0.001)
%                break;
%            end
%        end  
   end  
end
  function Losspicture1(Loss,MaxIter)
        X_label=1:1:MaxIter;
        plot(X_label,Loss(1,:),'.-','color','#FF0000','LineWidth', 1.5,'MarkerSize', 8);
        %,X,Loss(2,:),'m',X,Loss(3,:),'y'
        xlabel('Number of iterations','Fontsize',14,'Fontname','Times New Roman');
        ylabel('Objective function value','Fontsize',14,'Fontname','Times New Roman');
